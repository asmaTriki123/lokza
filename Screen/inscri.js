...
//const {Nom_station, type_lavage, ville, adresse, longitude, latitude, email} = userInfo

const FormSubmitButton = ({title, onPress}) =>{
  return (
    <TouchableOpacity onPress={onPress} style={styles.container}><Text style={{ fontSize: 18, color: '#fff'}}>{title}</Text></TouchableOpacity>
  )
}

const isValidForm = () =>{
  if(isValidObjField(userInfo)) return  updateError('required all failed', setError)
  if(!Nom_station.trim() || Nom_station.length >3) return updateError('Invalid Name', setError)
    ...
    {error? <Text style={{color: red, fontSize: 18, textAlign: 'center'}}>{error}</Text>: null}
      ...
      <View  >
       ...
       title='Nom_station'
        ...
        onChangeText={(value) => handleOnChangeText(value, 'Nom_station')}
        value={Nom_station}
        />
    </View>
    <View  >
...
alignSelf:'center'}}>
          ...
          <TouchableOpacity >
            <Text style={styles.link} onPress={FormSubmitButton}>Login</Text>
          </TouchableOpacity>
        </View>
